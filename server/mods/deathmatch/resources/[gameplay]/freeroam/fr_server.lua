...
g_RPCFunctions = {
    addPedClothes = {
        option = 'clothes',
        descr = 'Modifying clothes'
    },
    addVehicleUpgrade = {
        option = 'upgrades',
        descr = 'Adding/removing upgrades'
    },
    fadeVehiclePassengersCamera = true,
    fixVehicle = {
        option = 'repair',
        descr = 'Repairing vehicles'
    },
    giveMeVehicles = {
        option = 'createvehicle',
        descr = 'Creating vehicles'
    },
    giveMeWeapon = {
        option = 'weapons.enabled',
        descr = 'Getting weapons'
    },
    removePedClothes = {
        option = 'clothes',
        descr = 'Modifying clothes'
    ...
    setPedAnimation = {
        option = 'anim',
        descr = 'Setting an animation'
    },
    setPedFightingStyle = {
        option = 'setstyle',
        descr = 'Setting fighting style'
    },
    setPedGravity = {
        option = 'gravity.enabled',
        descr = 'Setting gravity'
    },
    setPedWalkingStyle = {
        option = 'walkstyle',
        descr = 'Setting walking style'
    },
    setPedStat = {
        option = 'stats',
        descr = 'Changing stats'
    },
    setVehicleColor = true,
    setVehicleHeadLightColor = true,
    setVehicleOverrideLights = {
        option = 'lights',
        descr = 'Forcing lights'
    },
    setVehiclePaintjob = {
        option = 'paintjob',
        descr = 'Applying paintjobs'
    },
...
function onLocalSettingChange(setting, value)
    ...
    if setting ~= "passive" or type(value) ~= "boolean" then
        return
    end
    if not g_PlayerData[client] then
        return
    end
...
local function getResponsiblePlayer(attacker)
    ...
    if getElementType(attacker) == "vehicle" then
        return getVehicleController(attacker)
    end
    return nil
end
...
addEventHandler("onVehicleDamage", root, function(_, attacker)
...
local settingsToSend = {"command_spam_protection", "tries_required_to_trigger",
                        "tries_required_to_trigger_low_priority", "command_spam_ban_duration",
                        "command_exception_commands", "removeHex", "spawnmapondeath", "weapons/kniferestrictions",
                        "kill", "warp", "hidecolortext", "gamespeed/enabled", "gamespeed/min", "gamespeed/max",
...
addEventHandler('onLoadedAtClient', resourceRoot, function()
    -- hide map on join
...
function handleClothesInit()
    ...
    for type = 0, 17 do
        ...
        while texture do
            table.insert(typeGroup.children, {
                id = index,
                texture = texture,
                model = model
            })
...
addEventHandler('onPlayerGravInit', root, function()
...
function setMySkin(skinid)
    ...
    if getElementModel(client) == skinid then
        return
    end
            ...
            if g_Trailers[vehID] then
                if getOption('vehicles.maxidletime') >= 0 then
                    if getOption('vehicles.idleexplode') then
                        g_VehicleData[vehicle].timers.fire =
                            setTimer(commitArsonOnVehicle, getOption('vehicles.maxidletime'), 1, vehicle)
                    end
                    g_VehicleData[vehicle].timers.destroy = setTimer(unloadVehicle, getOption('vehicles.maxidletime') +
                        (getOption('vehicles.idleexplode') and 10000 or 0), 1, vehicle)
                end
            end
        end
    else
...
addEventHandler('onVehicleExit', root, function(player, seat)
    ...
    if not g_VehicleData[source].timers.fire then
        ...
        if getOption('vehicles.maxidletime') >= 0 then
            if getOption('vehicles.idleexplode') then
                g_VehicleData[source].timers.fire = setTimer(commitArsonOnVehicle, getOption('vehicles.maxidletime'), 1,
                    source)
            end
            g_VehicleData[source].timers.destroy = setTimer(unloadVehicle, getOption('vehicles.maxidletime') +
                (getOption('vehicles.idleexplode') and 10000 or 0), 1, source)
        end
    end
...
addEventHandler('onServerCall', resourceRoot, function(fnName, ...)
    ...
    if fnName == "setElementAlpha" and isPassive(client) then
        return
    end
    ...
    if fnInfo and ((type(fnInfo) == 'boolean' and fnInfo) or (type(fnInfo) == 'table' and getOption(fnInfo.option))) then
        ...
        if not g_RPCFunctionsValidation[fnName] then
            outputDebugString('Function ' .. tostring(fnName) ..
                                  ' is missing from the validation list. Please add this one to g_RPCFunctionsValidation.',
                1)
            return
        end
        if not g_RPCFunctionsValidation[fnName](...) then
