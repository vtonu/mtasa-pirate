g_RPCFunctionsValidation = {
    addPedClothes = function(thePlayer, clothesTexture, clothesModel, clothesType, ...)
        if client ~= thePlayer then
            return false
        end
        if type(clothesTexture) ~= "string" then
            return false
        end
        if type(clothesModel) ~= "string" then
            return false
        end
        if type(clothesType) ~= "number" then
            return false
        end
        if (#{...} > 0) then
            return false
    ...
    addVehicleUpgrade = function(theVehicle, upgrade, ...)
        if getPedOccupiedVehicle(client) ~= theVehicle then
            return false
        end
        if type(upgrade) ~= "number" and type(upgrade) ~= "string" then
            return false
        end
        if (#{...} > 0) then
            return false
    ...
    setElementInterior = function(thePlayerOrVehicle, interior, ...)
        if client ~= thePlayerOrVehicle and thePlayerOrVehicle ~= getPedOccupiedVehicle(client) then
            return false
        end
        if type(tonumber(interior)) ~= "number" then
            return false
        end
        if (#{...} > 0) then
            return false
    ...
    setElementPosition = function(element, x, y, z)
        if client ~= element and element ~= getPedOccupiedVehicle(client) then
            return false
        end
    ...
    setPedAnimation = function(thePlayer, block, anim, time, loop, updatePosition, ...)
        ...
        if (block == false and #{anim, time, loop, updatePosition, ...} == 0) then
            return true
        end
        if block ~= nil and type(block) ~= "string" then
            return false
        end
        if anim ~= nil and type(anim) ~= "string" then
            return false
        end
        if time ~= nil and type(time) ~= "number" then
            return false
        end
        if loop ~= nil and type(loop) ~= "boolean" then
            return false
        end
        if updatePosition ~= nil and type(updatePosition) ~= "boolean" then
            return false
        end
        if (#{...} > 0) then
            return false
    ...
    setVehicleColor = function(theVehicle, r1, g1, b1, r2, g2, b2, r3, g3, b3, r4, g4, b4, ...)
        ...
        if r1 and (type(r1) ~= "number" or type(g1) ~= "number" or type(b1) ~= "number") then
            return false
        end
        -- Starting the next look up at g2, because for the Palette format which uses 4 arguments.
        if g2 and (type(r2) ~= "number" or type(g2) ~= "number" or type(b2) ~= "number") then
            return false
        end
        if r3 and (type(r3) ~= "number" or type(g3) ~= "number" or type(b3) ~= "number") then
            return false
        end
        if r4 and (type(r4) ~= "number" or type(g4) ~= "number" or type(b4) ~= "number") then
            return false
        end
        if (#{...} > 0) then
            return false
