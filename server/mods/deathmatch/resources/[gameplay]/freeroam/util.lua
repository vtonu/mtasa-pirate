...
addEventHandler('onClientCall', resourceRoot, function(fnName, ...)
    local fn = _G
...
function _addXMLChildrenToTable(parentNode, leafName, leafAttrs, targetTable)
    ...
    while leafNode do
        ...
        for k, attr in ipairs(leafAttrs) do
            leaf[attr] = (attr == 'id' and tonumber(xmlNodeGetAttribute(leafNode, attr)) or
                             xmlNodeGetAttribute(leafNode, attr))
        end
        i = i + 1
