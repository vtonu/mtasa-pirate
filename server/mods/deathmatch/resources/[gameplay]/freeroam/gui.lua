...
local function onProtectedClick(btn, state, x, y)
    ...
    if isFunctionOnCD(g_protectedElements[source]) then
        return
    end
...
function _planWindow(wnd, baseWnd, parentWnd, x, y, maxHeightInLine)
    ...
    if wndClass ~= 'br' then
        ...
        if not wnd.width then
            wnd.width = (classInfo[wndClass].width or (8 * text:len()) +
    ...
    if wnd.controls then
        ...
        for id, controlwnd in pairs(wnd.controls) do
            _, childX, childY, childMaxHeightInLine = _planWindow(controlwnd, baseWnd or wnd, wnd, childX, childY,
                childMaxHeightInLine)
        end
...
function _buildWindow(wnd, baseWnd, parentWnd)
    ...
    elseif wndClass == 'chk' then
        elem = guiCreateCheckBox(relX, relY, relWidth, relHeight, wnd.text or wnd.id or '', false, true,
            parentWnd.element)
    ...
    elseif wndClass == 'txt' then
        ...
        elem = _G['guiCreate' .. classInfo[wndClass].className](relX, relY, relWidth, relHeight,
            wnd.text or wnd.id or '', true, parentWnd.element)
        ...
        clickhandler = function()
            wndInput = {
                ...
                controls = {{
                    ...
                    id = 'ok',
                    onclick = function()
                        wnd.inputbox.callback(getControlText(wndInput, 'input'))
                        closeWindow(wndInput)
                    end
                }, {
                    'btn',
                    id = 'cancel',
...
function bindGridListToTable(...)
    ...
    if not listdata.clickHandlersSet then
        ...
        if gridListControlData.onitemclick then
            if gridListControlData.ClickSpamProtected then
                addEventHandler('onClientGUIClick', gridlist, function()
                    if isFunctionOnCD(gridListControlData.onitemclick) then
                        return
                    end
        ...
        addEventHandler('onClientGUIDoubleClick', gridlist, function()
            ...
            if tonumber(selData) == 0 then
                ...
                local clickedNode = followTreePath(listData.data, listData.currentPath,
                    table.map(string.split(selData, '/'), tonumber))
                if clickedNode[1] == 'group' then
                    ...
                    if gridListControlData.onitemdoubleclick then
                        if gridListControlData.DoubleClickSpamProtected then
                            if isFunctionOnCD(gridListControlData.onitemdoubleclick) then
                                return
                            end
            ...
            applyToLeaves(previousGroup, function(item)
                item.row = nil
            end)
            _updateBindedGridList(gridlist, listData, expandLastLevel)
        end, false)
    ...
    if #modifiableCols then
        local mt = {
            ...
            __newindex = function(leaf, k, v)
                if k ~= 'row' and leaf.row then
                    leaf.shadow[k] = v
                    guiGridListSetItemText(gridlist, leaf.row, table.find(columns, k),
                        type(v) == 'boolean' and (v and 'X' or '') or tostring(v), false, false)
        ...
        for i, item in ipairs(toDisplay) do
            if type(item) == 'table' then
                ...
                if item[1] == 'group' then
                    -- group
                    if expandLastLevel and (listdata.data.maxSubDepth == 2 or item.depth > 1) and item.maxSubDepth ==
                        item.depth + 1 then
...
function _putLeafInGridListRow(gridlist, row, leaf, columnAttrs)
    for k, attr in ipairs(columnAttrs) do
        guiGridListSetItemText(gridlist, row, k, type(leaf[attr]) == 'boolean' and (leaf[attr] and 'X' or '') or
            ('    ' .. tostring(leaf[attr])), false, false)
    end
end
