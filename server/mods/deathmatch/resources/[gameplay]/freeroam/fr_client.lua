...
local function getPassiveOwner(element)
	...
	if getElementType(element) == "player" then return element end
	if getElementType(element) == "vehicle" then return getVehicleController(element) end
	return nil
end
...
local function updatePassiveCollisions(forceCollidable)
	...
	for index, firstElement in ipairs(elements) do
		for secondIndex = index + 1, #elements do
			...
			local collidable = forceCollidable or not (isPassiveElement(firstElement) or isPassiveElement(secondElement))
...
wndMain = {
	...
	controls = {
		-- PLAYER
		{'lbl', text='PLAYER:', width=55},
		{'btn', id='kill', onclick=killLocalPlayer, text='KILL', width=65},
		{'br'},
		{'br'},

		-- PERKS
		{'lbl', text='PERKS:', width=55},
		{'lbl', id='perk', text='None', width=172},
		{'br'},

		-- MODE
		{'lbl', text='MODE:', width=55},
		{'lbl', id='modeblip', text='●', width=12},
		{'lbl', id='mode', text='Unprotected', width=160},
		{'br'},

		-- STATUS
		{'lbl', text='STATUS:', width=55},
		{'lbl', id='status', text='Walking on foot.', width=190},
		{'br'},

		-- LOCATION
		{'lbl', text='LOCATION:', width=65},
		{'lbl', id='location', text='Unknown.', width=190},
		{'br'},
		{'br'},

		-- VEHICLE OPTIONS
		{'lbl', text='VEHICLE OPTIONS:', width=240, align='left'},
		{'br'},

		-- VEHICLE MODIFICATION OPTIONS
		{'btn', id='repair', onclick=repairVehicle, text='REPAIR', width=62},
		{'btn', id='flip', onclick=flipVehicle, text='FLIP', width=52},
		{'btn', id='upgrades', window=wndUpgrades, text='UPGRADES', width=72},
		{'br'},

		-- VEHICLE AESTHETIC OPTIONS
		{'btn', id='color', onclick=openColorPicker, text='COLOR', width=92},
		{'btn', id='paintjob', window=wndPaintjob, text='PAINTJOB', width=92},
		{'br'},
		{'br'},

		-- PASSIVE MODE
		{'chk', id='passive', text='ENABLE PASSIVE MODE', onclick=togglePassiveMode}
	},
