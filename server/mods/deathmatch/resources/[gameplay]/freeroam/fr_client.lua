...
local server = setmetatable({}, {
    __index = function(t, k)
        t[k] = function(...)
            triggerServerEvent('onServerCall', resourceRoot, k, ...)
        end
        return t[k]
    end
})
...
function applyAnimation(leaf)
    ...
    if type(leaf) ~= "table" then
        ...
        if (not leaf) or (not leaf.parent.name) or (not leaf.name) or (string.len(leaf.name) > 25) or
            (string.len(leaf.parent.name) > 25) then
...
function animCmd(command, lib, name)
    ...
    if lib and name and ((lib:lower() == "finale" and name:lower() == "fin_jump_on") or
        (lib:lower() == "finale2" and name:lower() == "fin_cop1_climbout")) then
...
function giveWeaponCommand(cmd, weapon, amount)
    ...
    if weapon and string.len(weapon) > 25 then
        errMsg("Invalid weapon name/ID!")
        return
    end
    if amount and string.len(amount) > 9 then
        errMsg("Invalid amount!")
        return
    end
    ...
    if not weapon then
        errMsg("Invalid weapon! Syntax: /wp [weapon id/name]")
        return
    end
    if not amount or amount < 1 or amount > 999999999 or weapon < 1 or weapon > 46 then
        return
    end
    if internallyBannedWeapons[weapon] then
        return
    end
    if isPlayerAiming(localPlayer) then
        errMsg("You can't get weapons while aiming a weapon or driveby'ing!")
        return
    end
    if isPedReloadingWeapon(localPlayer) then
        errMsg("You can't use this command while reloading a weapon!")
        return
    end
    if weapon == 39 or weapon == 40 then
        errMsg("You can't get Satchels with /wp command! Use F1 > weapons instead!")
        return
    end
...
function clothesInit_cl(clothes)
    ...
    for i, typeGroup in ipairs(g_Clothes) do
        for j, cloth in ipairs(typeGroup.children) do
            ...
            cloth.wearing = clothes.playerClothes[typeGroup.type] and clothes.playerClothes[typeGroup.type].texture ==
                                cloth.texture and clothes.playerClothes[typeGroup.type].model == cloth.model or false
        end
...
wndClothes = {
    ...
    controls = {{
        ...
        id = 'addremove',
        width = 60,
        onclick = applyClothes,
        ClickSpamProtected = true
    }, {
        'btn',
        id = 'outfits',
        onclick = function()
            createWindow(wndOutfits)
        end
    }, {
        'btn',
        id = 'close',
...
function addClothesCommand(cmd, type, model, texture)
    ...
    if (not type or string.len(type) > 30) or (not model or string.len(model) > 30) or
        (not texture or string.len(texture) > 30) then
...
addEventHandler('onClientPlayerGravInit', resourceRoot, function(curgravity)
...
function warpUpdate()
    ...
    local players = table.map(getPlayersByPartName(text), function(p)
...
function initStats()
    applyToLeaves(getGridListCache(wndStats, 'statslist'), function(leaf)
        leaf.value = getPedStat(localPlayer, leaf.id)
    end)
end
...
wndWalking = {
    ...
    controls = {{
        ...
        'btn',
        id = 'Apply',
        onclick = applyWalkStyle,
        width = 70
    }, {
        'btn',
        id = 'Remove',
        onclick = stopWalkStyle,
        width = 70
    }, {
        'btn',
        id = 'Close',
...
function loadBookmarks()
    ...
    for i, child in ipairs(xmlNodeGetChildren(xml) or {}) do
        ...
        bookmarks[row + 1] = {tonumber(xmlNodeGetAttribute(child, "x")), tonumber(xmlNodeGetAttribute(child, "y")),
                              tonumber(xmlNodeGetAttribute(child, "z"))}
    end
    xmlUnloadFile(xml)
end
...
function setPlayerPosition(x, y, z, skipDeadCheck)
    ...
    if not hit then
        ...
        if isTimer(g_TeleportMatrixTimer) then
            killTimer(g_TeleportMatrixTimer)
        end
        ...
        if isTimer(g_TeleportTimer) then
            killTimer(g_TeleportTimer)
        end
...
function updatePlayerBlips()
    ...
    for elem, player in pairs(g_PlayerData) do
        if not player.gui.mapBlip then
            ...
            player.gui.mapBlip = guiCreateStaticImage(0, 0, 9, 9, elem == localPlayer and 'img/localplayerblip.png' or
                'img/playerblip.png', false, mapControl)
...
wndSetPos = {
    ...
    controls = {{
        ...
        width = g_MapSide,
        height = g_MapSide,
        onclick = fillInPosition,
        ondoubleclick = setPosClick,
        DoubleClickSpamProtected = true
    }, {
        'txt',
        id = 'x',
        ...
        'lbl',
        text = 'Right click on map to close'
    }},
    oncreate = setPosInit,
...
function setPosCommand(cmd, x, y, z, r)
    if isPlayerAiming(localPlayer) then
        return errMsg("You can't use /sp while aiming a weapon!")
    end
    if isPedReloadingWeapon(localPlayer) then
        return errMsg("You can't use /sp while reloading a weapon!")
    end
    ...
    x, y, z, r = string.gsub(x or "", ",", " "), string.gsub(y or "", ",", " "), string.gsub(z or "", ",", " "),
        string.gsub(r or "", ",", " ")
...
wndSpawnMap = {
    ...
    controls = {{
        ...
        width = g_MapSide,
        height = g_MapSide,
        ondoubleclick = spawnMapDoubleClick
    }, {
        'lbl',
        text = 'Double click a location on the map to spawn.',
        width = g_MapSide - 60,
        align = 'center'
    }, {
        'btn',
...
wndSetInterior = {
    ...
    controls = {{
        ...
        rows = {
            xml = 'data/interiors.xml',
            attrs = {'name', 'posX', 'posY', 'posZ', 'world'}
        },
...
function createVehicleCommand(cmd, ...)
    ...
    vehID = getVehicleModelFromName(table.concat(args, " ")) or tonumber(args[1]) and math.floor(tonumber(args[1])) or
                false
...
function upgradesInit()
    ...
    for i, upgrade in ipairs(getVehicleCompatibleUpgrades(vehicle)) do
        ...
        table.insert(group.children, {
            id = upgrade,
            installed = table.find(installedUpgrades, upgrade) ~= false
        })
    end
    table.sort(compatibleUpgrades, function(a, b)
        return a.name < b.name
    end)
...
function setPaintjobCommand(cmd, paint)
    ...
    if not paint then
        errMsg("Enter paintjob ID please!")
        return
    end
...
local function updateModeDisplay(state)
    ...
    local modeBlip = getControl(wndMain, "modeblip")
    if modeBlip and isElement(modeBlip) then
        guiLabelSetColor(modeBlip, 255, state and 255 or 0, 0)
    end
end
...
local function getPassiveOwner(element)
    ...
    if getElementType(element) == "vehicle" then
        return getVehicleController(element)
    end
    return nil
end

local function isPassiveElement(element)
    local player = getPassiveOwner(element)
    return player and g_PlayerData[player] and g_PlayerData[player].passive == true
end

local function updatePassiveCollisions(forceCollidable)
    ...
    for index, firstElement in ipairs(elements) do
        for secondIndex = index + 1, #elements do
            ...
            local collidable = forceCollidable or
                                   not (isPassiveElement(firstElement) or isPassiveElement(secondElement))
            setElementCollidableWith(firstElement, secondElement, collidable)
            setElementCollidableWith(secondElement, firstElement, collidable)
        end
    end
end
...
function updateGUI()
    ...
    if city and city ~= "Unknown" and city ~= zone then
        location = zone .. ", " .. (cityAbbreviations[city] or city) .. "."
    end

    setControlText(wndMain, "location", location)

    -- UPDATE CURRENT STATUS
    local vehicle = getPedOccupiedVehicle(localPlayer)

    if vehicle and isElement(vehicle) then
        setControlText(wndMain, "status", "Driving in " .. getVehicleName(vehicle) .. ".")
    else
        setControlText(wndMain, "status", "Walking on foot.")
    end
end
...
function onEnterVehicle(vehicle, seat)
    ...
    setTimer(updatePassiveCollisions, 50, 1)
end
addEventHandler('onClientPlayerVehicleEnter', root, onEnterVehicle)

function onExitVehicle(vehicle, seat)
    if (eventName == "onClientPlayerVehicleExit" and source == localPlayer) or
        (eventName == "onClientElementDestroy" and getElementType(source) == "vehicle" and
            getPedOccupiedVehicle(localPlayer) == source) then
        setControlText(wndMain, 'status', 'Walking on foot.')
        hideControls(wndMain, 'repair', 'flip', 'upgrades', 'color', 'paintjob', 'lightson', 'lightsoff')
        closeWindow(wndUpgrades)
...
wndMain = {
    ...
    }, {'br'}, -- LOCATION
    {
        ...
        text = 'Unknown.',
        width = 190
    }, {'br'}, {'br'}, -- VEHICLE OPTIONS
    {
        'lbl',
        text = 'VEHICLE OPTIONS:',
        width = 240,
        align = 'left'
    }, {'br'}, -- VEHICLE MODIFICATION OPTIONS
    {
        'btn',
        id = 'repair',
...
function joinHandler(player)
    ...
    g_PlayerData[player or source] = {
        name = getPlayerName(player or source),
        gui = {}
    }
end
...
addEventHandler('onClientResourceStop', root, function(stoppingResource)
    local editorResource, raceResource = getResourceFromName('editor'), getResourceFromName('race')
...
local function onLocalSettingChange(key, value)
    ...
    if key == "passive" and source == localPlayer then
        ...
        if checkbox and isElement(checkbox) then
            guiCheckBoxSetSelected(checkbox, value)
        end
    end
    if key == "passive" then
