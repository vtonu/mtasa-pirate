...
function dxDrawRoundedRectangle(radius,x,y,w,h,color,postGUI,subPixel,noTL,noTR,noBL,noBR)
    local noTL = not noTL and dxDrawCircle(x+radius,y+radius,radius,180,270,color,color,9,1,postGUI)
    local noTR = not noTR and dxDrawCircle(x+w-radius,y+radius,radius,270,360,color,color,9,1,postGUI)
    local noBL = not noBL and dxDrawCircle(x+radius,y+h-radius,radius,90,180,color,color,9,1,postGUI)
    local noBR = not noBR and dxDrawCircle(x+w-radius,y+h-radius,radius,0,90,color,color,9,1,postGUI)
    dxDrawRectangle(x+radius-(not noTL and radius or 0),y,w-2*radius+(not noTL and radius or 0)+(not noTR and radius or 0),radius,color,postGUI,subPixel)
    dxDrawRectangle(x,y+radius,w,h-2*radius,color,postGUI,subPixel)
    dxDrawRectangle(x+radius-(not noBL and radius or 0),y+h-radius,w-2*radius+(not noBL and radius or 0)+(not noBR and radius or 0),radius,color,postGUI,subPixel)
end

-- Check if cursor is inside a rectangle
...
-- Render the shader editor panel
function renderFunc()
...
-- Cleanup when player leaves vehicle
function cleanUp()
...
-- Handle shader panel mouse clicks
addEventHandler("onClientClick",root,function(btn,st)
    ...
    if btn == "left" then
        ...
        local w = settings["panelSize"][1]-20
        if st == "down" then
            if isMouseInPosition(posx,settings["panelPos"][2]+settings["panelSize"][1]-5,25,25) then
                ...
                return
            elseif isMouseInPosition(posx+w-25,settings["panelPos"][2]+settings["panelSize"][1]-5,25,25) then
                currentShader = currentShader+1
            ...
            if holded then
                ...
                local posy = settings["panelPos"][2]+settings["panelSize"][1]+#scrolls*25+30
                if isMouseInPosition(posx,posy,w,25) then
                    local vehicle = getPedOccupiedVehicle(localPlayer)
...
-- Handle shader data updates on vehicles
...
-- Set shader values from sliders
